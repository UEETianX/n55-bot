...
async def handle_function(event: Event):
    ...
    f=pd.read_csv("C:\\Users/Administrator/Desktop/Qbot/n55/src/plugins/duijue/dayship.txt")
    if time in np.array(f.loc[:,"A"]).tolist():
        await dayship.finish("今日已抽取过每日船只,输入[查模拟对决]试试运气吧~")
    else:
        shiptable = pd.read_csv("C:\\Users/Administrator/Desktop/Qbot/n55/src/plugins/duijue/战斗船指数.csv")
        ...
        f.to_csv("C:\\Users/Administrator/Desktop/Qbot/n55/src/plugins/duijue/dayship.txt",index = False)
        path = "C:\\Users/Administrator/Desktop/Qbot/n55/src/plugins"+links
        await dayship.finish("今天的幸运船只为"+ship+MessageSegment.image(path)+"输入[查模拟对决]试试运气吧~")
...
async def handle_function(event: Event):
    ...
    today = datetime.datetime.now()
    time = str(today.year) + "_" + str(today.month) + "_" + str(today.day) + "_" + QQ + group_id
    f=pd.read_csv("C:\\Users/Administrator/Desktop/Qbot/n55/src/plugins/duijue/daybat.txt")
    if time in np.array(f.loc[:,"A"]).tolist():
    ...
    def add_alpha_channel(img):
        ...
        return img_new
    card_path = "C:\\Users/Administrator/Desktop/Qbot/n55/src/plugins/duijue/duijuemuban.png"
    card = cv2.imread(card_path, cv2.IMREAD_UNCHANGED)
    shippic1 = cv2.imread("C:\\Users/Administrator/Desktop/Qbot/n55/src/plugins/duijue"+link1, cv2.IMREAD_UNCHANGED)
    shippic2 = cv2.imread("C:\\Users/Administrator/Desktop/Qbot/n55/src/plugins/duijue"+link2, cv2.IMREAD_UNCHANGED)
    if card.shape[2] == 3:
    ...
    fontpath = "C:\\Users/Administrator/Desktop/Qbot/n55/src/plugins/duijue/143zhengkuchaojihei.ttf"
    b,g,r,a = 0,0,0,0
    ...
    cv2.imwrite('C:\\Users/Administrator/Desktop/Qbot/n55/src/plugins/duijue/duijue.jpg', img)
    path = os.path.split(os.path.realpath(__file__))[0] + "/duijue.jpg"
    f=pd.read_csv("C:\\Users/Administrator/Desktop/Qbot/n55/src/plugins/duijue/daybat.txt")
    p = pd.DataFrame({"A":[time]})
    f = f._append(p,ignore_index = True,sort = False)
    f.to_csv("C:\\Users/Administrator/Desktop/Qbot/n55/src/plugins/duijue/daybat.txt",index = False)
    await daybat.finish(MessageSegment.image(path)+"仅供娱乐，别问XXX为啥打不过XXX")
