...
async def handle_function(args: Message = CommandArg()):
    df = pd.read_csv('C:\\Users\\Administrator\\Desktop\\Qbot/n55-q/src/plugins/船只价格表.csv',index_col=None,header=None)
    df.index=df.loc[:,0]
    df.columns=["ship","a","P"]
    ...
    if len(pd.to_numeric(re.split("-| |_|,|，",args)))==1:
        ...
        try:
            ...
            await shipP.finish("格式输错了吧~查价格100-150就行啦~"+MessageSegment.file_image(Path(__file__).parent / path))
...
#        print(y)
    ...
    await shipP.finish(MessageSegment.file_image(Path(__file__).parent / path))
