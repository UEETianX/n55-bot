...
async def handle_function(bot:Bot,event:Event,state:T_State,args: Message = CommandArg()):
    ...
    df = pd.read_csv('C:\\Users\\Administrator\\Desktop\\Qbot/n55-q/src/plugins/weapons.csv',index_col=None,header=0)
    if  args.lower()=="wb":
        args="asdsadawea"
    try:
        ...
        try:
            df = pd.read_csv('C:\\Users\\Administrator\\Desktop\\Qbot\\n55-q/src/plugins/weapons.csv',index_col=None,header=0)
            ...
            return
#            await weaponinfo.finish("是不是输错啦?如果发现武器没收录可以@UEE_TianXing哦~"+MessageSegment.file_image(Path(__file__).parent / path))
        ...
        if x>0:
            ...
            cv2.imwrite('C:\\Users\\Administrator\\Desktop\\Qbot\\n55-q/src/plugins/p.jpg', img)
            path=os.path.split(os.path.realpath(__file__))[0] + '/p.jpg'
            await weaponinfo.finish( MessageSegment.file_image(Path(__file__).parent / path))
    if len(args) >0:
        ...
        date = cv2.imread("C:\\Users\\Administrator\\Desktop\\Qbot\\n55-q/src/plugins/wuqimoban.png", cv2.IMREAD_UNCHANGED)
        ...
        fontpath = "C:\\Users\\Administrator\\Desktop\\Qbot\\n55-q/src/plugins/1637505678825667.ttc"
        link = "C:\\Users\\Administrator\\Desktop\\Qbot\\n55-q/src/plugins"+str(df.loc[df['CN-Name'].str.contains(args,case=False),"pic"].to_list()[0])
        ...
        img = np.array(img_pil)
        cv2.imwrite('C:\\Users\\Administrator\\Desktop\\Qbot/n55-q/src/plugins/wuqi.jpg', img)
