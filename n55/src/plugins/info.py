...
async def _(bot:Bot,event:Event,state:T_State):
    ...
    if  "查车辆" in msg or "查地面" in msg or "查载具" in msg:
        ...
        await find.finish(MessageSegment.image(path)+MessageSegment.image(path2))
    ...
    if "查舰队规模".lower() in msg.lower():
        ...
        if group_id == "774655949":
            ...
            await find.finish(MessageSegment.image(path1)+MessageSegment.image(path2)+MessageSegment.image(path3)+MessageSegment.image(path4))
        else:
    ...
    if "查活动".lower() in msg.lower():
        path=os.path.split(os.path.realpath(__file__))[0] + '/img/huodong.png'
        await find.finish(MessageSegment.image(path)+"https://b23.tv/tYBIUSx")
    ...
    if "查美元价格".lower() in msg.lower() or "查美金价格".lower() in msg.lower() or "查美刀价格".lower() in msg.lower():
        ...
        await find.finish("圣盾、铁砧价格表："+MessageSegment.image(path1)+"RSI、十字军价格表："+MessageSegment.image(path2)+"起源、联合外域、外星科技价格表："+MessageSegment.image(path3)+"德雷克、盾博尔、灰猫价格表："+MessageSegment.image(path4)+"武藏(未来)、南船座、克鲁格价格表："+MessageSegment.image(path5))
    ...
    if "原神".lower() == msg.lower() or "原神！".lower() == msg.lower():
        ...
        await find.finish("启动！"+MessageSegment.image(path))
    ...
    if "查".lower() == msg.lower():
        ...
        await find.send(MessageSegment.record(path1))
        await find.finish("说词啊！"+MessageSegment.image(path))
    ...
    if "查KOOK".lower() in msg.lower() or "查舰队KOOK".lower() in msg.lower() or "查舰队语音".lower() in msg.lower() or "查语音".lower() in msg.lower():
        ...
        elif group_id == "958822717" or group_id == "961650372":
    ...
    if "查部门".lower() in msg.lower() or  "查舰队部门".lower() == msg.lower():
        ...
        if group_id == "955342491":
            ...
            await find.finish("天命舰队成员登记表https://docs.qq.com/sheet/DTmx5S2N4YUNrbllX?tab=BB08J2"+ MessageSegment.image(path)+"【<星际公民>天命舰队部门介绍-哔哩哔哩】\nhttps://b23.tv/YWLMhb6\n舰队一级作战部门 武装部 后勤部 探索部 医疗部\n舰队二级行政部门 外交部 宣传部 信息资源部 教导组\n所有新加入成员可先加入一级作战部门后，经过考察后方可加入二级行政部门")
        else:
            path=os.path.split(os.path.realpath(__file__))[0] + '/FOD问答资料图/舰队规模.jpg'
    ...
    if  "查大蟑螂".lower() == msg.lower():
        if group_id == "546860192":
            await find.finish("本群未开放此功能")
        path=os.path.split(os.path.realpath(__file__))[0] + '/img/白痴.jpg'
        await find.finish("大蟑螂有人找你！" + MessageSegment.at(819613985) + MessageSegment.image(path))

    if  "查咸鱼".lower() == msg.lower():
        if group_id == "546860192":
            await find.finish("本群未开放此功能")
        path=os.path.split(os.path.realpath(__file__))[0] + '/img/白痴.jpg'
        await find.finish("醒醒嘞！该翻面了！" + MessageSegment.at(2833530830) + MessageSegment.image(path))
    ...
    if  "查梦心".lower() == msg.lower() or  "插梦心".lower() == msg.lower():
        ...
        if time=="3_15":
            path=os.path.split(os.path.realpath(__file__))[0] + '/img/生日.png'
            await find.finish("一股不明力量汇聚于梦心丹田，梦心虎虎生威，反向爆炒" + MessageSegment.at(event.user_id) +"，但谁知道这股力量会不会持续到明天呢～"+ MessageSegment.image(path))
        path=os.path.split(os.path.realpath(__file__))[0] + '/img/白痴.jpg'
    ...
    if  "插55姬" == msg or "插机器人" == msg:
        ...
        if n > 90:
            ...
            await find.finish(msg+MessageSegment.image(path))
        else:
    ...
    if "查55姬" == msg or "查机器人" == msg:
        ...
        await find.send(MessageSegment.audio(path=path1))
    ...
    if "查团子" == msg or "插团子" == msg:
        path=os.path.split(os.path.realpath(__file__))[0] + '/img/日喵.png'
        await find.finish("虽然有点难以启齿~但是~"+MessageSegment.at(912494733) +MessageSegment.image(path))
    ...
    if "查876" == msg or "插876" == msg:
        if group_id == "546860192":
            await find.finish("本群未开放此功能")
        path=os.path.split(os.path.realpath(__file__))[0] + '/img/白痴.jpg'
        await find.finish(Message("是求救信号，876该干活了")+MessageSegment.at(1726875126) + MessageSegment.image(path))
    ...
    if "查标枪残骸" == msg.lower() or "查景点1" == msg.lower():
        ...
        await find.finish("标枪残骸位置：\n十字军(Crusader)-戴马尔(Daymar)\nOM1: 577.0 km\nOM2: 459.4 km\nOM3: 204.5 km\nOM4: 708.6 km\nOM5: 429.2 km\nOM6: 599.8 km\n当前高度: 待补充\n上传者: UEE_TianXing\n预览图:\n"+MessageSegment.image(path1)+MessageSegment.image(path2)+MessageSegment.image(path3)+MessageSegment.image(path4))
    ...
    if "测试" == msg:
        ...
        await find.finish(MessageSegment.image(path).extract_plain_text())
    ...
    if group_id == "955342491" or group_id == "881232934" or group_id == "473081444":
        if "查武装部" == msg or "查武装部门" == msg:
            path=os.path.split(os.path.realpath(__file__))[0] + '/img/天命武装部.jpg'
            await find.finish("武装联合作战部门\n为舰队成员及其活动争夺太空与星球轨道的制空权，地面占领，进攻，防御为主要活动任务"+MessageSegment.image(path))

        if "查探索部" == msg or "查探索部门" == msg:
            path=os.path.split(os.path.realpath(__file__))[0] + '/img/天命探索部.png'
            await find.finish("科研探索联合部门\n为舰队成员提供行星资源探索，星图制作，旅游攻略等提供相应的资源支持舰队各部门运转！"+MessageSegment.image(path))
    
        if "查医疗部" == msg or "查医疗部门" == msg:
            path=os.path.split(os.path.realpath(__file__))[0] + '/img/天命医疗部.png'
            await find.finish("医疗创伤联合部门\n支持对舰队成员及其他有需要的人员进行人道主义支援行动，包括健康顾问、战场急救、假肢安装等相应的医疗支援！"+MessageSegment.image(path))
    
        if "查后勤部" == msg or "查后勤部门" == msg:
            path=os.path.split(os.path.realpath(__file__))[0] + '/img/天命后勤部.jpg'
            await find.finish("后勤联合保障部门\n负责舰队内的日常运作，同时提供舰队内物资搬运、工程建设、后勤补给等保障！"+MessageSegment.image(path))
    
        if "查外交部" == msg or "查外交部门" == msg:
            path=os.path.split(os.path.realpath(__file__))[0] + '/img/天命联合部.png'
            await find.finish("舰队联合外交部门\n向广大玩家提供游戏内外的外交合作，国际支援帮助，舰队级交流等一系列外交活动！"+MessageSegment.image(path))
    
        if "查教导部" == msg or "查教导部门" == msg or "查教导组" == msg:
            path=os.path.split(os.path.realpath(__file__))[0] + '/img/天命教导部.png'
            await find.finish("舰队教导组\n培训新人游戏玩法，传达游戏理念，增加对游戏的粘性和对舰队的忠诚度，提高玩家游戏兴趣，降低游戏门槛，从而让新人玩家更加快速的，高质量的融入游戏和舰队大家庭。"+MessageSegment.image(path))
    
        if "查指挥部" == msg or "查指挥部门" == msg:
            path=os.path.split(os.path.realpath(__file__))[0] + '/img/天命指挥部.png'
            await find.finish("舰队指挥部\n舰队的中枢部门，为舰队制定政策、日常决策、重大事项，战场指挥，部门协调等指挥功能"+MessageSegment.image(path))
    
        if "查宣传部" == msg or "查宣传部门" == msg:
            path=os.path.split(os.path.realpath(__file__))[0] + '/img/天命宣传部.png'
            await find.finish("舰队宣传部\n提供舰队所有的文宣素材，包装，软装，新闻宣传，舆论引导等宣传工作"+MessageSegment.image(path))
    
        if "查信息部" == msg or "查信息部门" == msg:
            path=os.path.split(os.path.realpath(__file__))[0] + '/img/天命信息部.png'
            await find.finish("舰队信息资源部\n保护舰队网络安全，日常维护应答机器人（55姬），提供舰队所需的网络资源等"+MessageSegment.image(path))
    ...
    if "抽奖" == msg:
        ...
        if str(user_id) in np.array(f.loc[:,"id"].astype(str)).tolist():
            ...
            f.to_csv("C:\\Users\\Administrator\\Desktop\\Qbot\\n55/src/plugins/choujiang/kucun.txt",index = False)
        else:
            text = [",".join(ship)]
...
#            n = choice(range(1,101))
        ...
        cv2.imwrite('C:\\Users\\Administrator\\Desktop\\Qbot\\n55/src/plugins/choujiang/choujiang.jpg', img)
        path = 'C:\\Users\\Administrator\\Desktop\\Qbot\\n55/src/plugins/choujiang' + "/choujiang.jpg"
        await find.finish(talk+MessageSegment.at(event.user_id) +"\n"+MessageSegment.image(path))
    ...
    if "随机老婆" == msg:
        if "T" == "T":
            ...
            try:
                ...
                image.save('C:\\Users\\Administrator\\Desktop\\Qbot\\n55/src/plugins/pictmp'+str(n)+'.png')
            except:
                sleep(1)
            path='C://Users/Administrator/Desktop/Qbot/n55/src/plugins/pictmp'+str(n)+'.png'
            await find.finish(MessageSegment.image(path))
    if "影集" == msg or "色图" == msg:
        await find.finish("易造成卡死，暂停使用")
        if "T" == "T":
...
#                url = "https://img.lapernum.site/jpg/Star_Citizen_"+nn+".jpg"
                ...
                resized_image.save('C:\\Users\\Administrator\\Desktop\\Qbot\\n55/src/plugins/picbang'+str(x)+'.png')
#                image.save('C:\\Users\\Administrator\\Desktop\\Qbot\\n55/src/plugins/picbang'+str(x)+'.png')
            except:
                sleep(1)
            path='C://Users/Administrator/Desktop/Qbot/n55/src/plugins/picbang'+str(x)+'.png'
            await find.finish(MessageSegment.image(path)+"原图请访问："+url0+"\nby:拉邦那 Lapernum")
...
#                image.save('C:\\Users\\Administrator\\Desktop\\Qbot\\n55/src/plugins/picbang'+str(x)+'.png')
    ...
    if msg.lower() =="查wb" or msg.lower() =="查wbccu" or msg.lower() =="查战争债券" or msg.lower() =="查沃邦德":
        ...
        if text2=="":
            ...
            await find.finish(MessageSegment.image(path)+"当前商店无WB出售")
        x =WBtable.shape[0]
...
#            await find.finish("你守住了阵线(但没守住钱包)"+MessageSegment.image(path))
    ...
    if "机器人挂了" in msg or "55姬挂了" in msg or "55挂了" in msg or "机器人死了" in msg or "55姬死了" in msg or "55死了" in msg:
        path=os.path.split(os.path.realpath(__file__))[0] + '/img/生气.jpg'
        await find.finish("你才挂了！"+MessageSegment.image(path))
