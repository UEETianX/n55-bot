...
async def handle_function(event: Event,args: Message = CommandArg()):
    ...
    if group_id == "859774647":
        await yaoqingma.finish("本群未开放此功能")
    print(user_id)
    df = pd.read_csv('C:\\Users\\Administrator\\Desktop\\Qbot\\n55/src/plugins/yaoqingma.csv',index_col=None,header=None)
    df.index=df.loc[:,0]
    df.columns=["ID","STAR"]
    ...
    if args and args.startswith("录入"):
        ...
        if str(user_id) in str(df.ID.tolist()):
            df.loc[df.ID == int(user_id),"STAR"] = STAR
            df.to_csv('C:\\Users\\Administrator\\Desktop\\Qbot\\n55/src/plugins/yaoqingma.csv',index = None,header = None)
            print("loc")
        else:
        ...
        if user_id in str(df.ID.tolist()):
            msg = str(df.loc[df.ID == int(user_id),"STAR"].tolist()[0])
        else:
