...
async def _(event:MessageEvent,matched: Dict[str, Any] = RegexDict()) -> None:
    ...
    if cont == "美元" or cont == "美刀" or cont == "刀" or cont == "美金":
        ...
        await exchange.finish(msg + MessageSegment.reply(event.msgSeq,sender_uin = event.user.id) )
    try:
        result = CN*float(num)
        print(str('%.2f'%result)+cont)
        msg = str('%.2f'%result)+"人民币"
        if int(result)<3500:
            await exchange.finish(msg + MessageSegment.at(event.user_id) )
        elif int(result) <7000:
            path=os.path.split(os.path.realpath(__file__))[0] + '/img/huilv500.jpg'
            await exchange.finish(msg + MessageSegment.at(event.user_id)+MessageSegment.image(path) )
        else:
